(function(){
    ...
    app.controller('all', function($scope, $http){
        ...
        $scope.delete = function(entity) {
            if (confirm('You really want to delete this interest?')) {
                $http.delete('/api/interest/'+entity.id, JSON.stringify($scope.entity), {headers: HEADERS})
    ...
    function swap(i, j, $scope, $http) {
        ...
        $http.post('/api/interest/swap', JSON.stringify({ids: [a.id, b.id]}), {headers: HEADERS})
            .then(function (response) {
                if (response.data.result) {
                    var temp = a.position;
